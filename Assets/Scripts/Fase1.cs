...
public class Fase1 : MonoBehaviour {
	...
	public PainelEstrela painelEstrela;
	...
	List <int> LinhasMove = new List <int> ();
	List <int> ColunaMove = new List <int> ();

	public List <char> SinalLinha = new List<char> (); 
	public List <char> SinalColuna = new List<char> ();
	...
	void CriaCaminho(){
		...
		for(int i = 0; i < SinalLinha.Count; i++){

			if(SinalLinha[i] == 'M' && SinalColuna[i] == 'M'){
 
				if ((LinhaPlayer + LinhasMove [i]) > 5   ||  (ColunaPlayer + ColunaMove[i]) > 11) {  // se tem

					if ((LinhaPlayer + LinhasMove [i]) > 5 && (ColunaPlayer + ColunaMove [i]) > 11) { // caso seja os dois
						Destinos.Add (Matriz [5, 11].gameObject);
						LinhaPlayer = 5;
						ColunaPlayer = 11;

					} else if ((LinhaPlayer + LinhasMove [i]) > 5) {  // linha maior
						
						Destinos.Add (Matriz [5, ColunaPlayer + ColunaMove [i]].gameObject);
						LinhaPlayer = 5;
						ColunaPlayer = ColunaPlayer + ColunaMove [i];
					} else {  // coluna maior
						
						Destinos.Add (Matriz [LinhaPlayer + LinhasMove [i], 11].gameObject);
						LinhaPlayer = LinhaPlayer + LinhasMove [i];
						ColunaPlayer = 11;
					}
					...
					Destinos.Add (Matriz [LinhaPlayer + LinhasMove [i], ColunaPlayer + ColunaMove [i]].gameObject);
					LinhaPlayer = LinhaPlayer + LinhasMove [i];
					ColunaPlayer = ColunaPlayer + ColunaMove [i];
				}
			}else if(SinalLinha[i] == 'M' && SinalColuna[i] == 'm'){

				if ((LinhaPlayer + LinhasMove [i]) > 5 || (ColunaPlayer - ColunaMove [i]) < 0) {  // se tem

					if ((LinhaPlayer + LinhasMove [i]) > 5 && (ColunaPlayer - ColunaMove [i]) < 0) { // caso seja os dois
						Destinos.Add (Matriz [5, 0].gameObject);
						LinhaPlayer = 5;
						ColunaPlayer = 0;
						...
						Destinos.Add (Matriz [LinhaPlayer + LinhasMove [i], 0].gameObject);
						LinhaPlayer = LinhaPlayer + LinhasMove [i];
						ColunaPlayer = 0;
					}
				...
				if ((LinhaPlayer - LinhasMove [i]) < 0 || (ColunaPlayer + ColunaMove [i]) > 11) {  // se tem

					if ((LinhaPlayer - LinhasMove [i]) < 0 && (ColunaPlayer + ColunaMove [i]) > 11) { // caso seja os dois
						Destinos.Add (Matriz [0, 11].gameObject);
						LinhaPlayer = 0;
						ColunaPlayer = 11;

					} else if ((LinhaPlayer - LinhasMove [i]) < 0) {  // linha maior

						Destinos.Add (Matriz [0, ColunaPlayer + ColunaMove [i]].gameObject);
						LinhaPlayer = 0;
						ColunaPlayer = ColunaPlayer + ColunaMove [i];
				...
				if ((LinhaPlayer - LinhasMove [i]) < 0 || (ColunaPlayer - ColunaMove [i]) < 0) {  // se tem

					if ((LinhaPlayer - LinhasMove [i]) < 0 && (ColunaPlayer - ColunaMove [i]) < 0) { // caso seja os dois
						Destinos.Add (Matriz [0, 0].gameObject);
						LinhaPlayer = 0;
						ColunaPlayer = 0;
	...
	void ProximoPasso(){
		...
		if (indiceDestino < SinalLinha.Count) {
	...
	void CaminhaEntreOsPontos(){  // caminha até o proximo destino
		if (play == true && vivo == true) {
			...
			if (Jogador.transform.position == Toca.transform.position) {
				...
				if (Player.ContCenouras == NumCenouras) {
